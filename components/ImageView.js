...
function ImageView({ navigation }) {
  ...
  const onRefresh = () => {
    console.log("OnRefresh Get Called...");
    setRefreshing(true);
    fetchWallPapers()
  };
  ...
  const fetchWallPapers = () => {
    axios.get(ENDPOINTS.GET_ALL_WALLPAPER + "ACTIVE", { headers: getHeaders })
      .then(response => {
        for (let i = 0; i < response.data.video_data.length; i++) {
          const item = response.data.video_data[i];
          console.log("URL : ", response.data.video_data[i].FilePath);
          response.data.video_data[i].FilePath = item.FilePath.replace("localhost", "192.168.0.109");
        }
        setWallPapers(response.data.video_data)
      })
      .catch(error => {
  ...
  const explore_image = item => {
    navigation.navigate('Explore', { path: item.FilePath, fileName: item.fileName });
  };
    ...
    return (
      <Card style={Styles.container}>
        <TouchableOpacity onPress={() => explore_image(item)}>
          <Card.Cover style={Styles.card_cover} source={{ uri: item.FilePath }} />
        </TouchableOpacity>
      </Card>
    );
  };
    ...
    <SafeAreaView style={Styles.parent_view}>
      ...
      >
        ...
        <View style={Styles.list}>
          {wallpapers.map(function (item, index) {
